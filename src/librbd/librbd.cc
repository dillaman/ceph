...
/* pool mirroring */
extern "C" int rbd_mirror_is_enabled(rados_ioctx_t p, int *enabled) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  bool is_enabled;
  int r = librbd::mirror_is_enabled(io_ctx, &is_enabled);
  if (r < 0) {
    return r;
  }
...
extern "C" int rbd_mirror_set_enabled(rados_ioctx_t p, int enabled) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  return librbd::mirror_set_enabled(io_ctx, enabled == 0);
}

extern "C" int rbd_mirror_peer_add(rados_ioctx_t p,
                                   const char *cluster_uuid,
                                   const char *cluster_name, int64_t pool_id,
                                   const char *client_name) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  return librbd::mirror_peer_add(io_ctx, cluster_uuid, cluster_name, pool_id,
                                 client_name);
}
