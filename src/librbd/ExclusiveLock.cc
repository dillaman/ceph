...
bool ExclusiveLock<I>::is_lock_owner() const {
  ...
  ldout(m_image_ctx.cct, 20) << __func__ << "=" << lock_owner << dendl;
  return lock_owner;
}
...
void ExclusiveLock<I>::handle_acquire_lock(int r) {
  ...
  if (r == -EBUSY) {
    ldout(cct, 5) << "unable to acquire exclusive lock, retrying" << dendl;
  } else if (r < 0) {
...
void ExclusiveLock<I>::send_release_lock() {
  ...
  m_lock.Unlock();
  m_image_ctx.aio_work_queue->block_writes(new C_ReleaseBlockWrites(this));
  m_lock.Lock();
}
...
void ExclusiveLock<I>::handle_release_lock(int r) {
  ...
  if (r >= 0) {
    ...
    if (!m_image_ctx.aio_work_queue->writes_empty()) {
