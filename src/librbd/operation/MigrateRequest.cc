...
private:
  ...
  bool is_within_head_bounds() {
    I &image_ctx = this->m_image_ctx;
    assert(image_ctx.snap_lock.is_locked());

    return image_ctx.size > 0 &&
      Striper::get_num_objects(image_ctx.layout, image_ctx.size) > m_object_no;
  }
  ...
  void migrate_object() {
    ...
    if (is_within_overlap_bounds()) {
      ...
      m_snap_map = image_ctx.migration_info.snap_map;
      if (m_snap_map[CEPH_NOSNAP][0] == CEPH_NOSNAP &&
          !is_within_head_bounds()) {
        m_snap_map.erase(CEPH_NOSNAP);
      }
      assert(!m_snap_map.empty());

      auto req = deep_copy::ObjectCopyRequest<I>::create(
        image_ctx.parent, &image_ctx, m_snap_map,
        m_object_no, ctx);
...
uint64_t MigrateRequest<I>::get_num_overlap_objects() {
  ...
  auto total = image_ctx.size;
  for (auto it : image_ctx.migration_info.snap_map) {
    auto snap_ids = it.second;
    auto snap_id = snap_ids[0];
    auto size = image_ctx.get_image_size(snap_id);
    total = std::max(total, size);
  }

  return total > 0 ?
    Striper::get_num_objects(image_ctx.layout, total) : 0;
}
