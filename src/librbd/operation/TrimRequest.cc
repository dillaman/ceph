...
public:
  ...
  virtual int send() {
    ...
    auto req = new io::AioObjectTrim(&image_ctx, oid, m_object_no, m_snapc,
                                     this, false);
    req->send();
    return 0;
  }
...
void TrimRequest<I>::send_clean_boundary() {
  ...
  for (vector<ObjectExtent>::iterator p = extents.begin();
    ...
    io::AioObjectRequest<> *req;
    if (p->offset == 0) {
      req = new io::AioObjectTrim(&image_ctx, p->oid.name, p->objectno, snapc,
                                  req_comp, true);
    } else {
      req = new io::AioObjectTruncate(&image_ctx, p->oid.name, p->objectno,
