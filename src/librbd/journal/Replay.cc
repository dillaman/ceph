...
void Replay<I>::shut_down(bool cancel_ops, Context *on_finish) {
  ...
  if (flush_comp != nullptr) {
    ...
    io::AioImageRequest<I>::aio_flush(&m_image_ctx, flush_comp);
  }
...
void Replay<I>::flush(Context *on_finish) {
  ...
  RWLock::RLocker owner_locker(m_image_ctx.owner_lock);
  io::AioImageRequest<I>::aio_flush(&m_image_ctx, aio_comp);
}
...
void Replay<I>::handle_event(const journal::AioDiscardEvent &event,
  ...
  bool flush_required;
  io::AioCompletion *aio_comp = create_aio_modify_completion(
    on_ready, on_safe, io::AIO_TYPE_DISCARD, &flush_required);
  io::AioImageRequest<I>::aio_discard(&m_image_ctx, aio_comp, event.offset,
                                      event.length);
  if (flush_required) {
    m_lock.Lock();
    io::AioCompletion *flush_comp = create_aio_flush_completion(nullptr);
    m_lock.Unlock();
...
void Replay<I>::handle_event(const journal::AioWriteEvent &event,
  ...
  bool flush_required;
  io::AioCompletion *aio_comp = create_aio_modify_completion(on_ready, on_safe,
                                                             io::AIO_TYPE_WRITE,
                                                             &flush_required);
  io::AioImageRequest<I>::aio_write(&m_image_ctx, aio_comp, event.offset,
...
template <typename I>
io::AioCompletion *Replay<I>::create_aio_modify_completion(
    Context *on_ready, Context *on_safe, io::aio_type_t aio_type,
    bool *flush_required) {
  Mutex::Locker locker(m_lock);
  CephContext *cct = m_image_ctx.cct;
  ...
  *flush_required = (m_aio_modify_unsafe_contexts.size() ==
  ...
  // on_safe callback
  io::AioCompletion *aio_comp = io::AioCompletion::create_and_start<Context>(
