...
template <typename I>
void CloseRequest<I>::send_close_object_map() {
  if (m_image_ctx->object_map == nullptr) {
    send_flush();
    return;
  }

  // since exclusive lock wasn't active, we must be read a snapshot
  CephContext *cct = m_image_ctx->cct;
  ldout(cct, 10) << this << " " << __func__ << dendl;
...
template <typename I>
void CloseRequest<I>::handle_close_object_map(int r) {
  CephContext *cct = m_image_ctx->cct;
  ldout(cct, 10) << this << " " << __func__ << ": r=" << r << dendl;
