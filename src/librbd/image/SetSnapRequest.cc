  ...
  if (!refresh_parent) {
    if (m_snap_id == CEPH_NOSNAP) {
      ...
      *result = apply();
      ...
      return send_refresh_object_map(result);
    }
  }
...
template <typename I>
Context *SetSnapRequest<I>::send_refresh_object_map(int *result) {
  ...
  using klass = SetSnapRequest<I>;
  Context *ctx = create_context_callback<
    klass, &klass::handle_refresh_object_map>(this);
  m_object_map = new ObjectMap(m_image_ctx);
  m_object_map->refresh(m_snap_id, ctx);
  return nullptr;
}

template <typename I>
Context *SetSnapRequest<I>::handle_refresh_object_map(int *result) {
  CephContext *cct = m_image_ctx.cct;
  ldout(cct, 10) << __func__ << ": r=" << *result << dendl;
