...
int get_remote_pool_spec(const po::variables_map &vm,
                         const std::string &spec,
                         std::string *remote_client_name,
                         std::string *remote_cluster,
                         std::string *remote_pool) {
  ...
  if (vm.count("remote-pool")) {
    *remote_pool = vm["remote-pool"].as<std::string>();
  }
...
std::string get_pool_name(const std::string &config_path,
                          const std::string &client_name,
                          const std::string &cluster_name,
                          int64_t pool_id) {
  librados::Rados rados;
  librados::IoCtx io_ctx;
  int r = init_remote(config_path, client_name, cluster_name, "",
                      &rados, nullptr);
  if (r < 0) {
    goto err_pool_lookup;
  }

  r = rados.ioctx_create2(pool_id, io_ctx);
  if (r < 0) {
    goto err_pool_lookup;
  }
  return io_ctx.get_pool_name();

err_pool_lookup:
  return "unknown (" + stringify(pool_id) + ")";
}
...
void format_mirror_peers(const std::string &config_path,
  ...
  if (formatter != nullptr) {
    ...
    for (auto &peer : peers) {
      ...
      formatter->dump_string("pool_name", get_pool_name(config_path,
                                                        peer.client_name,
                                                        peer.cluster_name,
                                                        peer.pool_id));
    ...
    if (peers.empty()) {
      ...
      for (auto &peer : peers) {
        tbl << " "
            << peer.cluster_uuid
            << peer.cluster_name
            << get_pool_name(config_path, peer.client_name, peer.cluster_name,
                             peer.pool_id)
            << peer.client_name
            << TextTable::endrow;
      }
...
void get_peer_add_arguments(po::options_description *positional,
  ...
  positional->add_options()
    ("remote-pool-spec", "remote pool spec\n"
     "(example: [<client name>@]<cluster name>[/<pool name>]");
  options->add_options()
    ("remote-client-name", po::value<std::string>(), "remote client name")
    ("remote-cluster", po::value<std::string>(), "remote cluster name")
    ("remote-pool", po::value<std::string>(), "remote pool name");
}
...
int execute_peer_add(const po::variables_map &vm) {
  ...
  std::string remote_cluster;
  std::string remote_pool_name = pool_name;
  int r = get_remote_pool_spec(
    vm, utils::get_positional_argument(vm, arg_index),
    &remote_client_name, &remote_cluster, &remote_pool_name);
  if (r < 0) {
    return r;
  }
  ...
  r = init_remote(config_path, remote_client_name, remote_cluster,
                  remote_pool_name, &remote_rados, &remote_io_ctx);
  if (r < 0) {
    return r;
  }
  ...
  r = rbd.mirror_peer_add(io_ctx, remote_cluster_uuid, remote_cluster,
                          remote_io_ctx.get_id(), remote_client_name);
  if (r < 0) {
