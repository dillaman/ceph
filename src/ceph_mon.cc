...
/**
 * Check whether a monitor exists at g_conf->mon_data
 *
 * @return **0** if exists. -ENOENT if directory does not exist,
 *         -ENOTDIR if path exists but is not a directory and
 *         -EINVAL if directory is empty or not a monitor.
 */
int mon_may_exist()
{
  int err = mon_data_exists();
  if (err < 0)
    return err;
  ...
  if (err == -ENOTEMPTY)
    return 0;
  else if (err == 0) // mon_data is empty
    return -EINVAL;
  return err;
}
