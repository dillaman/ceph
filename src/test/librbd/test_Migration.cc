...
struct TestMigration : public TestFixture {
  ...
  void open_image(librados::IoCtx& io_ctx, const std::string &name,
    ...
    *ictx = new librbd::ImageCtx(name.c_str(), "", NULL, io_ctx, false);
    m_ictxs.insert(*ictx);
  ...
  void test_stress2(bool concurrent) {
    ...
    thread user([this]() {
        ...
        for (int i = 0; i < 0; i++) {
          for (int j = 0; j < 20; j++) {
            uint64_t off = j * m_ictx->size / 20;
            uint64_t len = m_ictx->size / 20 / (i + 2);
            write(off, len, '1' + i);
...
{
  // deep_copy::ObjectCopyRequest relays on ORDERSNAP flag for
  // concurrent writes, and librados_test_stub does not support it.
