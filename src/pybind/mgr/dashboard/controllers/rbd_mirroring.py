...
class RbdMirroringPoolMode(RESTController):
    ...
    @RbdMirroringTask('edit', {'pool_name': '{pool_name}'}, 5.0)
...
class RbdMirroringPoolPeer(RESTController):
    ...
    def set(self, pool_name, peer_uuid, cluster_name=None, client_id=None,
        ...
        ioctx = mgr.rados.open_ioctx(pool_name)
        if cluster_name:
            rbd.RBD().mirror_peer_set_cluster(peer_uuid, cluster_name)
        if client_id:
            rbd.RBD().mirror_peer_set_client(peer_uuid, 'client.{}'.format(client_id))
