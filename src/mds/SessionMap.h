...
private:
  ...
  map<version_t, list<MDSInternalContext*> > waitfor_flush; // flush session messages
public:
...
public:
  ...
  version_t wait_for_flush(MDSInternalContext* c) {
    waitfor_flush[get_push_seq()].push_back(c);
    return get_push_seq();
  }
  void finish_flush(version_t seq, list<MDSInternalContext*>& ls) {
...
public:  // i am lazy
  ...
  map<version_t, list<MDSInternalContext*> > commit_waiters;
...
public:
  ...
  // -- loading, saving --
  inodeno_t ino;
  list<MDSInternalContext*> waiting_for_load;
  ...
  void load(MDSInternalContext *onload);
  void _load_finish(int r, bufferlist &bl);
  void save(MDSInternalContext *onsave, version_t needv=0);
