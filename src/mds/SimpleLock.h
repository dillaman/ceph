...
public:
  ...
  void take_waiting(uint64_t mask, list<MDSInternalContext*>& ls) {
  ...
  void set_state_rejoin(int s, list<MDSInternalContext*>& waiters) {
  ...
  */
  ...
  void decode_state_rejoin(bufferlist::iterator& p, list<MDSInternalContext*>& waiters) {
    __s16 s;
    ::decode(s, p);
