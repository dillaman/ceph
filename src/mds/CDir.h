...
public:
  void split(int bits, list<CDir*>& subs, list<MDSInternalContext*>& waiters, bool replay);
  void merge(list<CDir*>& subs, list<MDSInternalContext*>& waiters, bool replay);
...
private:
  ...
  void finish_old_fragment(list<MDSInternalContext*>& waiters, bool replay);
 ...
 public:
  ...
  void fetch(MDSInternalContext *c, bool ignore_authpinnability=false);
  void fetch(MDSInternalContext *c, const std::string& want_dn, bool ignore_authpinnability=false);
protected:
  ...
  // -- commit --
  std::map<version_t, std::list<MDSInternalContext*> > waiting_for_commit;
...
public:
  ...
  void commit(version_t want, MDSInternalContext *c,
  ...
  // -- waiters --
protected:
  std::map< string_snap_t, std::list<MDSInternalContext*> > waiting_on_dentry;

public:
  bool is_waiting_for_dentry(const std::string& dname, snapid_t snap) {
    return waiting_on_dentry.count(string_snap_t(dname, snap));
  }
  void add_dentry_waiter(const std::string& dentry, snapid_t snap, MDSInternalContext *c);
  void take_dentry_waiting(const std::string& dentry, snapid_t first, snapid_t last, std::list<MDSInternalContext*>& ls);
  void take_sub_waiting(std::list<MDSInternalContext*>& ls);  // dentry or ino

  void add_waiter(uint64_t mask, MDSInternalContext *c);
  void take_waiting(uint64_t mask, std::list<MDSInternalContext*>& ls);  // may include dentry waiters
