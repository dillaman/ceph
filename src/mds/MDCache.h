...
public:
  ...
  // waiters
  map<int, map<inodeno_t, list<MDSInternalContext*> > > waiting_for_base_ino;

  void discover_base_ino(inodeno_t want_ino, MDSInternalContext *onfinish, int from=-1);
  void discover_dir_frag(CInode *base, frag_t approx_fg, MDSInternalContext *onfinish,
			 int from=-1);
  void discover_path(CInode *base, snapid_t snap, filepath want_path, MDSInternalContext *onfinish,
		     bool want_xlocked=false, int from=-1);
  void discover_path(CDir *base, snapid_t snap, filepath want_path, MDSInternalContext *onfinish,
...
public:
  ...
  void wait_for_uncommitted_master(metareqid_t reqid, MDSInternalContext *c) {
...
protected:
  ...
  list<MDSInternalContext*> rejoin_waiters;
...
private:
  ...
  list<MDSInternalContext*> waiting_for_open;
...
public:
  ...
  void create_empty_hierarchy(C_GatherBase<MDSInternalContext> *gather);
  void create_mydir_hierarchy(C_GatherBase<MDSInternalContext> *gather);

  bool is_open() { return open; }
  void wait_for_open(MDSInternalContext *c) {
    waiting_for_open.push_back(c);
  }

  void open_root_inode(MDSInternalContext *c);
  void open_root();
  void open_mydir_inode(MDSInternalContext *c);
  void populate_mydir();

  void _create_system_file(CDir *dir, const char *name, CInode *in, MDSInternalContext *fin);
  void _create_system_file_finish(MutationRef& mut, CDentry *dn,
                                  version_t dpv, MDSInternalContext *fin);

  void open_foreign_mdsdir(inodeno_t ino, MDSInternalContext *c);
  CDentry *get_or_create_stray_dentry(CInode *in);

  MDSInternalContext *_get_waiter(MDRequestRef& mdr, Message *req, MDSInternalContext *fin);
   ...
   */
  int path_traverse(MDRequestRef& mdr, Message *req, MDSInternalContext *fin, const filepath& path,
  ...
  void open_remote_dirfrag(CInode *diri, frag_t fg, MDSInternalContext *fin);
  ...
  void open_remote_dentry(CDentry *dn, bool projected, MDSInternalContext *fin,
			  bool want_xlocked=false);
  void _open_remote_dentry_finish(CDentry *dn, inodeno_t ino, MDSInternalContext *fin,
...
protected:
  ...
  MDSInternalContext* _open_ino_get_waiter(inodeno_t ino, MMDSOpenIno *m);
...
public:
  ...
  void open_ino(inodeno_t ino, int64_t pool, MDSInternalContext *fin,
  ...
  void find_ino_peers(inodeno_t ino, MDSInternalContext *c, int hint=-1);
...
public:
  ...
  CDir* add_replica_dir(bufferlist::iterator& p, CInode *diri, int from, list<MDSInternalContext*>& finished);
  CDir* forge_replica_dir(CInode *diri, frag_t fg, int from);
  CDentry *add_replica_dentry(bufferlist::iterator& p, CDir *dir, list<MDSInternalContext*>& finished);
  CInode *add_replica_inode(bufferlist::iterator& p, CDentry *dn, list<MDSInternalContext*>& finished);
...
private:
  ...
  void adjust_dir_fragments(CInode *diri, frag_t basefrag, int bits,
			    list<CDir*>& frags, list<MDSInternalContext*>& waiters, bool replay);
...
public:
  void wait_for_uncommitted_fragment(dirfrag_t dirfrag, MDSInternalContext *c) {
