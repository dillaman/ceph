...
void CInode::store(MDSInternalContext *fin)
{
...
void CInode::fetch(MDSInternalContext *fin)
{
  dout(10) << "fetch" << dendl;
...
void CInode::store_backtrace(MDSInternalContext *fin, int op_prio)
{
...
struct C_Inode_FragUpdate : public MDSInternalContext {
  ...
  C_Inode_FragUpdate(CInode *i, CDir *d, MutationRef& m) : MDSInternalContext(in->mdcache->mds), in(i), dir(d), mut(m) {}
  void finish(int r) {
...
void CInode::add_dir_waiter(frag_t fg, MDSInternalContext *c)
{
...
void CInode::take_dir_waiting(frag_t fg, list<MDSInternalContext*>& ls)
{
  if (waiting_on_dir.empty())
    return;

  map<frag_t, list<MDSInternalContext*> >::iterator p = waiting_on_dir.find(fg);
...
void CInode::add_waiter(uint64_t tag, MDSInternalContext *c) 
{
...
void CInode::take_waiting(uint64_t mask, list<MDSInternalContext*>& ls)
{
  if ((mask & WAIT_DIR) && !waiting_on_dir.empty()) {
    // take all dentry waiters
    while (!waiting_on_dir.empty()) {
      map<frag_t, list<MDSInternalContext*> >::iterator p = waiting_on_dir.begin();
...
void CInode::unfreeze_inode(list<MDSInternalContext*>& finished) 
{
...
void CInode::clear_ambiguous_auth(list<MDSInternalContext*>& finished)
{
...
void CInode::_decode_locks_rejoin(bufferlist::iterator& p, list<MDSInternalContext*>& waiters,
