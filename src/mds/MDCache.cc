 ...
 */
class MDCacheContext : public MDSInternalContext {
  protected:
  MDCache *mdcache;
...
void MDCache::create_empty_hierarchy(C_GatherBase<MDSInternalContext> *gather)
{
  // create root dir
...
void MDCache::create_mydir_hierarchy(C_GatherBase<MDSInternalContext> *gather)
{
  // create mds dir
  char myname[10];
...
struct C_MDC_CreateSystemFile : public MDCacheContext {
  ...
  C_MDC_CreateSystemFile(MDCache *c, MutationRef& mu, CDentry *d, version_t v, MDSInternalContext *f) :
...
void MDCache::_create_system_file(CDir *dir, const char *name, CInode *in, MDSInternalContext *fin)
{
...
void MDCache::_create_system_file_finish(MutationRef& mut, CDentry *dn, version_t dpv, MDSInternalContext *fin)
{
...
void MDCache::open_root_inode(MDSInternalContext *c)
{
...
void MDCache::open_mydir_inode(MDSInternalContext *c)
{
...
void MDCache::open_foreign_mdsdir(inodeno_t ino, MDSInternalContext *fin)
{
...
void MDCache::handle_mds_recovery(int who)
{
  ...
  list<MDSInternalContext*> waiters;
...
bool MDCache::open_undef_inodes_dirfrags()
{
  ...
  C_GatherBuilderBase<MDSInternalContext> gather(g_ceph_context, new C_MDC_RejoinGatherFinish(this));
  for (set<CDir*>::iterator p = fetch_queue.begin();
       p != fetch_queue.end();
       ++p) {
    CDir *dir = *p;
...
MDSInternalContext *MDCache::_get_waiter(MDRequestRef& mdr, Message *req, MDSInternalContext *fin)
{
  if (mdr) {
...
int MDCache::path_traverse(MDRequestRef& mdr, Message *req, MDSInternalContext *fin,     // who
 ...
 */
void MDCache::open_remote_dirfrag(CInode *diri, frag_t approxfg, MDSInternalContext *fin) 
{
...
struct C_MDC_OpenRemoteDentry : public MDCacheContext {
  ...
  bool want_xlocked;
  C_MDC_OpenRemoteDentry(MDCache *m, CDentry *d, inodeno_t i, MDSInternalContext *f, bool wx) :
...
void MDCache::open_remote_dentry(CDentry *dn, bool projected, MDSInternalContext *fin, bool want_xlocked)
{
...
void MDCache::_open_remote_dentry_finish(CDentry *dn, inodeno_t ino, MDSInternalContext *fin,
...
struct C_MDC_OpenInoTraverseDir : public MDSInternalContext {
  MDCache *cache;
  inodeno_t ino;
  public:
  C_MDC_OpenInoTraverseDir(MDCache *c, inodeno_t i) : MDSInternalContext(cache->mds), cache(c), ino(i) {}
  void finish(int r) {
...
MDSInternalContext* MDCache::_open_ino_get_waiter(inodeno_t ino, MMDSOpenIno *m)
{
  if (m)
...
void MDCache::open_ino(inodeno_t ino, int64_t pool, MDSInternalContext* fin,
 ...
 */
void MDCache::find_ino_peers(inodeno_t ino, MDSInternalContext *c, int hint)
{
			    ...
			    int from)
{
  ...
  if (base->is_ambiguous_auth()) {
    ...
    list<MDSInternalContext*> finished;
...
void MDCache::handle_discover_reply(MDiscoverReply *m) 
{
  ...
  */
  ...
  list<MDSInternalContext*> finished, error;
  int from = m->get_source().num();
...
CDentry *MDCache::add_replica_dentry(bufferlist::iterator& p, CDir *dir, list<MDSInternalContext*>& finished)
{
  string name;
  snapid_t last;
  ::decode(name, p);
...
CInode *MDCache::add_replica_inode(bufferlist::iterator& p, CDentry *dn, list<MDSInternalContext*>& finished)
{
  inodeno_t ino;
  snapid_t last;
  ::decode(ino, p);
...
void MDCache::_fragment_logged(MDRequestRef& mdr)
{
  ...
  C_GatherBuilderBase<MDSInternalContext> gather(g_ceph_context, new C_MDC_FragmentStore(this, mdr));
