...
void CDir::finish_old_fragment(list<MDSInternalContext*>& waiters, bool replay)
{
...
void CDir::split(int bits, list<CDir*>& subs, list<MDSInternalContext*>& waiters, bool replay)
{
...
void CDir::merge(list<CDir*>& subs, list<MDSInternalContext*>& waiters, bool replay)
{
...
void CDir::add_dentry_waiter(const string& dname, snapid_t snapid, MDSInternalContext *c) 
{
			       ...
			       list<MDSInternalContext*>& ls)
{
  ...
  map<string_snap_t, list<MDSInternalContext*> >::iterator p = waiting_on_dentry.lower_bound(lb);
...
void CDir::take_sub_waiting(list<MDSInternalContext*>& ls)
{
  dout(10) << "take_sub_waiting" << dendl;
  if (!waiting_on_dentry.empty()) {
    for (map<string_snap_t, list<MDSInternalContext*> >::iterator p = waiting_on_dentry.begin(); 
...
void CDir::add_waiter(uint64_t tag, MDSInternalContext *c) 
{
  // hierarchical?
...
void CDir::take_waiting(uint64_t mask, list<MDSInternalContext*>& ls)
{
  if ((mask & WAIT_DENTRY) && !waiting_on_dentry.empty()) {
    // take all dentry waiters
    while (!waiting_on_dentry.empty()) {
      map<string_snap_t, list<MDSInternalContext*> >::iterator p = waiting_on_dentry.begin(); 
...
// -----------------------
// FETCH
void CDir::fetch(MDSInternalContext *c, bool ignore_authpinnability)
{
  string want;
  return fetch(c, want, ignore_authpinnability);
}

void CDir::fetch(MDSInternalContext *c, const string& want_dn, bool ignore_authpinnability)
{
 ...
 */
void CDir::commit(version_t want, MDSInternalContext *c, bool ignore_authpinnability, int op_prio)
{
...
void CDir::_committed(version_t v)
{
  ...
  map<version_t, list<MDSInternalContext*> >::iterator p = waiting_for_commit.begin();
  while (p != waiting_for_commit.end()) {
    map<version_t, list<MDSInternalContext*> >::iterator n = p;
    ++n;
