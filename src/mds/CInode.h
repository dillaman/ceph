...
public:
  ...
  void clear_ambiguous_auth(std::list<MDSInternalContext*>& finished);
  ...
  void store(MDSInternalContext *fin);
  void _stored(version_t cv, Context *fin);
  void fetch(MDSInternalContext *fin);
  void _fetched(bufferlist& bl, bufferlist& bl2, Context *fin);  

  void build_backtrace(int64_t pool, inode_backtrace_t& bt);
  void store_backtrace(MDSInternalContext *fin, int op_prio=-1);
  ...
  // -- waiting --
protected:
  std::map<frag_t, std::list<MDSInternalContext*> > waiting_on_dir;
public:
  void add_dir_waiter(frag_t fg, MDSInternalContext *c);
  void take_dir_waiting(frag_t fg, std::list<MDSInternalContext*>& ls);
  bool is_waiting_for_dir(frag_t fg) {
    return waiting_on_dir.count(fg);
  }
  void add_waiter(uint64_t tag, MDSInternalContext *c);
  void take_waiting(uint64_t tag, std::list<MDSInternalContext*>& ls);
  ...
  void _decode_locks_rejoin(bufferlist::iterator& p, std::list<MDSInternalContext*>& waiters,
...
public:
  ...
  void unfreeze_inode(std::list<MDSInternalContext*>& finished);
  void unfreeze_inode();
