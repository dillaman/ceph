...
/**
 * XXX FIXME this is convenient for contexts that had a Server member
 * but no MDS member, but in the current implementation the resulting
 * child class ends up carrying around two pointers.
 */
class C_ServerContext : public MDSInternalContext {
  protected:
  Server *server;
  ...
  void complete(int r) {
    assert(server);
    assert(server->mds->mds_lock.is_locked_by_me());
    Context::complete(r);
  }
};
...
void Server::flush_client_sessions(set<client_t>& client_set, C_GatherBuilderBase<MDSInternalContext>& gather)
{
...
class C_MDS_TerminatedSessions : public MDSInternalContext {
  Server *server;
  void finish(int r) {
    server->terminating_sessions = false;
  }
  public:
  C_MDS_TerminatedSessions(Server *s) : MDSInternalContext(server->mds), server(s) {}
};
...
/*
 * XXX bump in the interface here, not using an MDSInternalContext here
 ...
 */
void Server::journal_and_reply(MDRequestRef& mdr, CInode *in, CDentry *dn, LogEvent *le, MDSInternalContext *fin)
{
...
void Server::submit_mdlog_entry(LogEvent *le, MDSInternalContext *fin, MDRequestRef& mdr,
                                const char *event)
{
  if (mdr) {
...
class C_MDS_TryFindInode : public MDSInternalContext {
  Server *server;
  MDRequestRef mdr;
public:
  C_MDS_TryFindInode(Server *s, MDRequestRef& r) : MDSInternalContext(s->mds), server(s), mdr(r) {}
  virtual void finish(int r) {
				    ...
				    bool no_lookup)    // true if we cannot return a null dentry lease
{
  ...
  if (r < 0) {  // error
    if (r == -ENOENT && n == 0 && mdr->dn[n].size()) {
      ...
      MDSInternalContext *c = new C_MDS_TryFindInode(this, mdr);
...
struct C_MDS_LookupIno2 : public MDSInternalContext {
  Server *server;
  MDRequestRef mdr;
  C_MDS_LookupIno2(Server *s, MDRequestRef& r) : MDSInternalContext(s->mds), server(s), mdr(r) {}
  void finish(int r) {
...
class C_MDS_SlaveLinkPrep : public MDSInternalContext {
  Server *server;
  MDRequestRef mdr;
  CInode *targeti;
public:
  C_MDS_SlaveLinkPrep(Server *s, MDRequestRef& r, CInode *t) :
    MDSInternalContext(s->mds), server(s), mdr(r), targeti(t) { }
  void finish(int r) {
    assert(r == 0);
...
class C_MDS_SlaveLinkCommit : public MDSInternalContext {
  Server *server;
  MDRequestRef mdr;
  CInode *targeti;
public:
...
struct C_MDS_CommittedSlave : public MDSInternalContext {
  Server *server;
  MDRequestRef mdr;
  C_MDS_CommittedSlave(Server *s, MDRequestRef& m) : MDSInternalContext(s->mds), server(s), mdr(m) {}
  void finish(int r) {
...
struct C_MDS_LoggedLinkRollback : public MDSInternalContext {
  Server *server;
  MutationRef mut;
  MDRequestRef mdr;
  C_MDS_LoggedLinkRollback(Server *s, MutationRef& m, MDRequestRef& r) : MDSInternalContext(s->mds), server(s), mut(m), mdr(r) {}
  void finish(int r) {
...
struct C_MDS_LoggedRenameRollback : public MDSInternalContext {
  Server *server;
  MutationRef mut;
  MDRequestRef mdr;
  CDentry *srcdn;
  ...
  C_MDS_LoggedRenameRollback(Server *s, MutationRef& m, MDRequestRef& r,
			     ...
			     CDentry *st, bool f) :
    MDSInternalContext(s->mds), server(s), mut(m), mdr(r), srcdn(sd), srcdnpv(pv), destdn(dd),
				...
				bool finish_mdr)
{
  ...
  if (mdr && !mdr->more()->slave_update_journaled) {
    ...
    MDSInternalContext *fin = new C_MDS_LoggedRenameRollback(this, mut, mdr, srcdn, srcdnpv,
						  destdn, straydn, finish_mdr);
    submit_mdlog_entry(le, fin, mdr, __func__);
    mdlog->flush();
  }
