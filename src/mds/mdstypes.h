...
protected:
 ...
 public:
  ...
  // waiting
 protected:
  multimap<uint64_t, MDSInternalContext*>  waiting;
 ...
 public:
  bool is_waiter_for(uint64_t mask, uint64_t min=0) {
    ...
    for (multimap<uint64_t,MDSInternalContext*>::iterator p = waiting.lower_bound(min);
	 p != waiting.end();
	 ++p) {
  ...
  virtual void add_waiter(uint64_t mask, MDSInternalContext *c) {
    if (waiting.empty())
      get(PIN_WAITER);
    waiting.insert(pair<uint64_t,MDSInternalContext*>(mask, c));
...
//			       << dendl;
  ...
  virtual void take_waiting(uint64_t mask, list<MDSInternalContext*>& ls) {
    if (waiting.empty()) return;
    multimap<uint64_t,MDSInternalContext*>::iterator it = waiting.begin();
...
//				   << dendl;
  ...
  virtual void add_lock_waiter(int type, uint64_t mask, MDSInternalContext *c) { assert(0); }
