...
protected:
  ...
  struct _pending_prepare {
    ...
    _pending_prepare(MDSInternalContext *c, version_t *pt, bufferlist *pb, bufferlist& m) :
  ...
  map<version_t, list<MDSInternalContext*> > ack_waiters;
...
public:
  ...
  void _prepare(bufferlist& mutation, version_t *ptid, bufferlist *pbl, MDSInternalContext *onfinish);
  ...
  void wait_for_ack(version_t tid, MDSInternalContext *c) {
