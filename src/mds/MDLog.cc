...
void MDLog::write_head(MDSInternalContext *c) 
{
  journaler->write_head(new C_IO_Wrapper(mds, c));
}
...
void MDLog::create(MDSInternalContext *c)
{
...
void MDLog::open(MDSInternalContext *c)
{
...
class C_ReopenComplete : public MDSInternalContext {
  MDLog *mdlog;
  MDSInternalContext *on_complete;
public:
  C_ReopenComplete(MDLog *mdlog_, MDSInternalContext *on_complete_) : MDSInternalContext(mdlog->mds), mdlog(mdlog_), on_complete(on_complete_) {}
  void finish(int r) {
    mdlog->append();
 ...
 */
void MDLog::reopen(MDSInternalContext *c)
{
  dout(5) << "reopen" << dendl;
...
void MDLog::_submit_entry(LogEvent *le, MDSInternalContext *c)
{
  assert(submit_mutex.is_locked_by_me());
...
void MDLog::wait_for_safe(MDSInternalContext *c)
{
  if (!g_conf->mds_log) {
    // hack: bypass.
    c->complete(0);
...
void MDLog::_journal_segment_subtree_map(MDSInternalContext *onsync)
{
  assert(submit_mutex.is_locked_by_me());
...
{
  C_GatherBuilderBase<MDSInternalContext> gather_bld(g_ceph_context);
...
void MDLog::replay(MDSInternalContext *c)
{
 ...
 */
void MDLog::_recovery_thread(MDSInternalContext *completion)
{
  assert(journaler == NULL);
 ...
 * safely completed.
 */
void MDLog::_reformat_journal(JournalPointer const &jp_in, Journaler *old_journal, MDSInternalContext *completion)
{
