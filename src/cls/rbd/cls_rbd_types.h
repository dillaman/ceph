...
inline void decode(MigrationHeaderType &type, bufferlist::iterator& it) {
  uint8_t int_type;
  using ceph::decode;
  decode(int_type, it);
...
struct MigrationSpec {
  ...
  MigrationSpec(MigrationHeaderType header_type, int64_t pool_id,
                ...
                bool mirroring, MigrationState state,
    ...
    : header_type(header_type), pool_id(pool_id), image_name(image_name),
      ...
      mirroring(mirroring), state(state), state_description(state_description) {
  }
